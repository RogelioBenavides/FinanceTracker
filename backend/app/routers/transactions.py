...
def get_transaction(transaction_id: int, db: Session = Depends(get_db)):
    tx = _q(db).filter(Transaction.id == transaction_id).first()
    if not tx:
...
def create_transaction(payload: TransactionCreate, db: Session = Depends(get_db)):
    ...
    if payload.card_id and not db.query(Card).filter(Card.id == payload.card_id).first():
    ...
    db.commit()
    return get_transaction(tx.id, db)
